...
class API:
    def __init__(self, publisher, fileobserver, profiles_manager,
                 extra_context, listen_address, port):
    ...
    async def start(self):
        logger.info(f'Starting API handler '
                    f'listening on {self._listen_address}:{self._port}')
        await self._runner.setup()
        self._site = web.TCPSite(
            self._runner, self._listen_address, self._port
        )
    ...
    async def _error_wrapper(self, request, handler):
        ...
        except (web.HTTPFound, web.HTTPForbidden, web.HTTPConflict,
                web.HTTPNotFound, web.HTTPUnauthorized, web.HTTPNoContent,
                web.HTTPCreated):
            raise
    ...
    def _resolve_rid(self, resource_id):
        return resource_id and \
               self._profiles_manager.get_resource_name(resource_id)
    ...
    async def _get_tasks(self, request):
        return web.json_response(
            self._publisher.get_enqueued_tasks(),
            dumps=CustomEncoder().encode
        )
    ...
    async def _get_profiles(self, request):
        ...
        return web.json_response(
            self._profiles_manager.profiles,
            dumps=CustomEncoder().encode
        )
    ...
    def _on_validation_error(self, error, req, schema, error_status_code,
                             error_headers):
    ...
    def _report_error(self, message, body, headers=None,
