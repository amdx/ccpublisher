...
class FileObserver:
    ...
    def __init__(self, file_path, backlog):
        ...
        self._watcher.watch(
            path=str(self._file_path.parent),
            flags=flags
        )
    ...
    def start(self):
        return asyncio.create_task(self._observer_task(),
                                   name='File observer task')
    ...
    async def _observer_task(self):
        ...
        if self._file_path.exists():
            ...
            logger.warning(f'File {self._file_path} does not exist, '
                           f'delaying opening')
            logfile = None
        ...
        while True:
            ...
            logger.debug(f'inotify event: {event} '
                         f'flags={aionotify.Flags.parse(event.flags)}')

            if event.name == self._file_path.name:
                if (aionotify.Flags.MODIFY & event.flags and
                        self._state == self.State.OPENED):
                    while True:
                ...
                elif ((aionotify.Flags.DELETE & event.flags or
                      aionotify.Flags.MOVED_FROM & event.flags)
                      and self._state == self.State.OPENED):
                    logger.info('Closing file since it has been '
                                'deleted or moved')
                    await logfile.close()
                    logfile = None
                    self._state = self.State.CLOSED
                elif (aionotify.Flags.CREATE & event.flags
                      and self._state in (self.State.CLOSED, self.State.INIT)):
